...
- **Node.js 兼容**：lifecycle 现可在 Node 22+ 运行——`src/` 为纯逻辑（无
  `Deno.*`、无 `IS_NODE` 分支），定时器类型已使用
  `ReturnType<typeof setTimeout>`（Deno/Bun 返回 number、Node 返回
  `NodeJS.Timeout`，跨运行时类型统一）；错误文案经 `@dreamer/i18n` v1.1.2、
...
- **src/**：无改动（纯逻辑包，`tests/mod.test.ts` 已在模块级调用
  `setLifecycleLocale("zh-CN")` 锁定中文文案，CI 英文 locale 下无需额外修复）。
